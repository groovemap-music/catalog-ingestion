...
async fn health_handler(State((state, _)): State<AppState>) -> (StatusCode, Json<serde_json::Value>) {
    ...
    let health = json!({
        ...
        "extraction_progress": {
            ...
            "release_groups": state.extraction_progress.release_groups,
        ...
        "last_extraction_time": {
            ...
            "release_groups": state.last_extraction_time.get(&crate::types::DataType::ReleaseGroups).map(|t| t.elapsed().as_secs_f64()),
...
async fn ready_handler(State((state, _)): State<AppState>) -> StatusCode {
    ...
    // Service is ready if it has started processing (running/completed) or
